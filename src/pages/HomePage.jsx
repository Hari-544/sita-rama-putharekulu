    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-yellow-50 text-gray-800">
      ...
      <header className="sticky top-0 z-50 bg-white/60 backdrop-blur-md shadow-md px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <h1 className="text-2xl font-extrabold text-orange-900 tracking-tight">SITA RAMA <span className="text-yellow-600">PUTHAREKULU</span></h1>

          <nav className="flex items-center gap-4">
            <a href="#products" className="text-gray-700 hover:text-orange-800 font-medium">Products</a>
            <a href="/reviews" className="text-gray-700 hover:text-orange-800 font-medium">Reviews</a>
            <button onClick={() => setShowCart(!showCart)} className="bg-orange-700 hover:bg-orange-800 text-white px-4 py-2 rounded-2xl font-semibold shadow">Cart ({cart.length})</button>
          </nav>
        </div>
      </header>
      ...
      <section className="relative py-16">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          <div>
            <h2 className="text-5xl font-extrabold text-orange-900 leading-tight">Handmade, Thin & Crispy<br /><span className="text-yellow-600">Traditional Putharekulu</span></h2>
            <p className="mt-6 text-lg text-gray-700 max-w-2xl">Crafted using time-honored recipes and premium ingredients — delivered fresh to your door. Try our bestsellers with dry fruits and jaggery.</p>

            <div className="mt-8 flex gap-4">
              <a href="#products" className="inline-flex items-center gap-3 bg-orange-700 hover:bg-orange-800 text-white px-6 py-3 rounded-3xl font-semibold shadow-lg">Shop Now</a>
              <a href="/reviews" className="inline-flex items-center gap-3 border border-orange-200 bg-white px-6 py-3 rounded-3xl font-medium hover:shadow">See Reviews</a>
            </div>

            <div className="mt-6 flex gap-4 flex-wrap text-sm text-gray-600">
              <span className="bg-white/60 px-3 py-1 rounded-full">Free shipping over ₹1000</span>
              <span className="bg-white/60 px-3 py-1 rounded-full">Freshly packed</span>
              <span className="bg-white/60 px-3 py-1 rounded-full">Secure payment</span>
            </div>
          </div>
          ...
          <div className="mx-auto">
            <div className="relative w-[420px] h-[300px] md:w-[520px] md:h-[360px] rounded-3xl overflow-hidden shadow-2xl">
              ...
              <div className="absolute -bottom-8 left-6 bg-white/90 px-4 py-2 rounded-2xl shadow transform rotate-2">
                <div className="font-bold text-orange-800">Authentic Atreyapuram Taste</div>
                <div className="text-xs text-gray-600">Limited daily batches — order early</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="products" className="py-16 px-6">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-bold text-center text-orange-800 mb-12">Our Products</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">

            {products.map((product) => (

              <div key={product.id} className="relative bg-white rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition">
                <div className="absolute top-4 right-4 bg-yellow-400 text-xs font-bold px-3 py-1 rounded-full">{product.sizes.split('|')[0]}</div>

                <img src={product.image} alt={product.name} className="h-56 w-full object-cover" />

                <div className="p-6">
                  <h3 className="text-2xl font-bold text-orange-900">{product.name}</h3>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <button onClick={() => addToCart(product)} className="bg-orange-700 hover:bg-orange-800 text-white px-5 py-2 rounded-2xl font-semibold transition">Add</button>
                  </div>
                </div>
              </div>
